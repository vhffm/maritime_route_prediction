...
def LEGACY_evaluate(model, prediction_task, test_paths, test_trajectories, network, n_start_nodes=1, MOGen_n_walks=100, MOGen_n_steps=1, eval_mode='path'):
    '''
    ...
    for i in range (0, len(test_paths)):
        ...
        if prediction_task == 'path':
            ...
            if model.type == 'MOGen':
                ...
                predicted_path, flag = model.predict_path(start, end_node)
        ...
        # Prediction task: next node(s)         
            ...
            if model.type == 'MOGen':
                try:
                    prediction = model.predict_next_nodes(start_node, G, n_steps=MOGen_n_steps, 
                                                          n_predictions=1, n_walks=MOGen_n_walks, verbose=False)  # predict next nodes
                except:
                    predicted_path, SSPD, distances, choice_accuracy = [], np.nan, np.nan, np.nan
                else:
                    predicted_path = [x for x in list(prediction)[0]]
                    true_path = true_path[0:n_start_nodes+MOGen_n_steps]
                    end_node = predicted_path[-1]
                    if predicted_path == true_path:
                        SSPD, distances = 0, [0]
                        choice_accuracy = 1.0
